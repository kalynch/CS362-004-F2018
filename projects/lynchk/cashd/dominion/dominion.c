...
//  Reveal cards from your deck until you draw two treasure cards
//  Put those cards in your hand and discard other revealed cards
...
//  Gain a treasure to your hand up to 3 more than it.
int playMine(int choice1, int choice2, int handPos, int currentPlayer, struct gameState *state)
{
  int i;
  int j = state->hand[currentPlayer][choice1]; //store card we will trash

  // Card to trash is not a treasure card
  if (state->hand[currentPlayer][choice1] < copper || state->hand[currentPlayer][choice1] > gold)
  {
    return -1;
  }

  // Card to gain is not a known card
  if (choice2 > treasure_map || choice2 < curse)
  {
    return -1;
  }

  // Value of trashed card + 3 is greater than cost of card to gain
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  case feast:
    return playFeast(handPos, currentPlayer, state);
  case gardens:
    return -1;
  case mine:
    return playMine(choice1, choice2, handPos, currentPlayer, state);
  case remodel:
